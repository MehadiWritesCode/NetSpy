...
def start_live_capture(interface, callback, stop_events):
    ...
    def process_packet(pkt):
        ...
        if pkt.haslayer(DHCP):
            ...
            dhcp_mac = pkt.addr2 if pkt.haslayer(Dot11) else (
                pkt[Ether].src if pkt.haslayer(Ether) else None
            )
        ...
        # Signal strength
        try:
            power = pkt[RadioTap].dBm_AntSignal if pkt.haslayer(RadioTap) else -100
        except:
            power = -100
        ...
        if pkt.haslayer(Dot11) and pkt.type == 2:
            ...
            if router_mac and client_mac and router_mac != client_mac:
                if client_mac not in found_clients or found_clients[client_mac][0] != power:
        ...
        if new_data_found:
            ...
            for b_id, info in found_ssids.items():
                ...
                router_data += (
                    f"{'Router':<12} | {b_id:<20} | {info[0]:^25} | {info[1]:^6} | "
                    f"{router_brand:^12} | {info[2]:^4} | {info[3]:^15} | "
                    f"{info[4]:^6} | {info[5]:^5} | {info[6]:^5}\n"
                )

            device_data = ""
            for m_adr, data in found_clients.items():
                pwr = data[0]
                mac_lower = m_adr.lower()
                host_name = host_names.get(mac_lower, "")
                device_prefix = m_adr[:8].upper()
                device_brand = VENDOR_DB.get(device_prefix, "Randomized")
                display_name = host_name if host_name else device_brand
